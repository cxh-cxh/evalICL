...
batch_path = "results/dp_t10003_20251109_011110_qwen3-vl-plus"
...
for result in results:
    ...
    for item in result:
        ...
        if item["difficulty"] == "easy":
            ...
            # alpha = (suc - succ["easy"] / total["easy"]) / 4 + 0.25 
            alpha = 0
            total["easy"] += 1 
            succ["easy"] += suc 
...
axs[1].set_title("var")
axs[1].plot(np.var(ratios["easy"],axis=0), color='green')
axs[1].plot(np.var(ratios["medium"],axis=0), color='yellow')
axs[1].plot(np.var(ratios["hard"],axis=0), color='red')
...
axs[2].plot(np.mean(scores,axis=0),label = "DARE pipeline")
axs[2].plot(np.mean(old_scores,axis=0), label = "Prevalent pipeline")
axs[2].fill_between(range(scores.shape[1]), 
                 np.min(scores,axis=0), 
                 np.max(scores,axis=0),
                 alpha=0.2, label='')
axs[2].fill_between(range(old_scores.shape[1]), 
...
# axs[2].add_line(line1)
# axs[2].add_line(line2)
# axs[2].text(x=0, y=0, s="Add Env1", fontsize=12, color="grey", 
#          ha="left", va="bottom")
# axs[2].text(x=51, y=0, s="Add Env2", fontsize=12, color="grey", 
#          ha="left", va="bottom")
# axs[2].text(x=101, y=0, s="Add Env3", fontsize=12, color="grey", 
#          ha="left", va="bottom")

plt.savefig(batch_path.split("/")[-1] + ".png")

print(np.mean(scores,axis=0)[-1])
