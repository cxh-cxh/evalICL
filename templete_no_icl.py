...
- Test case difficulty is evaluated with regard to the model. So observe the action of the robot and think about how difficult the test case is for the robot.
- The dynamic constraints of the robot. For example, an object blocked by another object might be hard to reach for the robot.
- The properties of the objects inside the box. Consider whether their shapes and positions are easy for the robot to grab. Notice that some transparent objects may be hard to recognize.
- The order of picking up objects. Consider whether the robot chooses the valid object to pick up first. The order of picking up objects is important since some objects may be obscurred or stuck.
- The lighting conditions. Too bright or too dark lighting might affect the model's performance.
- Action patterns. For example, too many retries on picking up a single object may indicate the test case is difficult.
...
example_templete_1 = Template("""[EXAMPLE START]
...
query_templete_1 = Template("""[QUERY START]
