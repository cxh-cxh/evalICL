...
parser.add_argument("--train_envs", nargs="*", type=str, default=[], help="train environments")
...
def build_examples_content(test_records, test_names):
    ...
    for i in range(len(test_records)):
        ...
        ex_desc_2 = example_templete_2.render(ex)
...
BASE_PROMPT = base_prompt_templete.render(k=args.k)
...
async def main():
    ...
    for run in range(args.count):
        ...
        for train_name in args.train_envs:
            data = load_jsonl("data/" + train_name + ".jsonl")
            train_records+=data
        ...
        all_records = all_records[args.starting_num : args.starting_num + args.queries_num]
        all_test_names = all_test_names[args.starting_num : args.starting_num + args.queries_num]

        for test_name in ["t10003_poison"]:
            poison_data = load_jsonl("data/" + test_name + ".jsonl")[20:]

        all_records += poison_data
        all_test_names += ["t10003_poison"] * len(poison_data)
        queries_num = args.queries_num + len(poison_data)
        ...
        #     for q in queries:
        #         fq.write(json.dumps([q_ for q_ in q if q_["type"] == 'text'], ensure_ascii=False) + "\n")
        # for q in queries:
