...
def load_queries(path="./new_test_cases_fixed.jsonl", q_size=30):
    data = []
    p = pathlib.Path(path)
    if not p.exists():
        return data
...
def load_examples(path_e="./8_10_test_case.jsonl", path_m="./3_7_test_case.jsonl", path_h="./0_1_2_test_case.jsonl",e_size = 15):
    data1 = []
    data2 = []
    data3 = []
    p_e = pathlib.Path(path_e)
    p_m = pathlib.Path(path_m)
    p_h = pathlib.Path(path_h)
    if not p_e.exists() or not p_m.exists() or not p_h.exists():
        return []
    with p_e.open("r", encoding="utf_8") as f1:
        for line1 in f1:
            line1 = line1.strip()
            if not line1:
                continue
            try:
                ...
                pass
    with p_m.open("r", encoding="utf_8") as f2:
        for line2 in f2:
            line2 = line2.strip()
            if not line2:
                continue
            try:
                ...
                pass
    with p_h.open("r", encoding="utf_8") as f3:
        for line3 in f3:
            line3 = line3.strip()
            if not line3:
                continue
    ...
    examples = data1[:e_size] + data2[:e_size] + data3[:e_size]
    return examples


queries=load_queries("./new_test_cases_fixed.jsonl")
examples = load_examples(e_size = 15)
...
async def main():
    ...
    output_dir = os.path.join(base_dir, timestamp + "_" + model_type + '_32')
