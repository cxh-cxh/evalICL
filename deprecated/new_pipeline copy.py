...
os.environ["OPENAI_API_KEY"] = "sk-5717b623e2574041b2be8879f7d44220"
...
def load_jsonl(path="./test_cases.jsonl", q_size=40, e_size=30):
    data = []
    p = pathlib.Path(path)
    if not p.exists():
        return data
...
all_records = []
all_test_names = []
for test_name in ["t10003_env1", "t10003_env2", "t10003_env3"]:
    data = load_jsonl("data/" + test_name + ".jsonl")
    all_records += data
...
retriever = Retriever(
    starting_test_records=all_records[:starting_num],
    starting_test_names=all_test_names[:starting_num],
    model_name=model_name,
    img_emb_path="data/img_emb.hdf5",
)
all_records = all_records[starting_num:]
all_test_names = all_test_names[starting_num:]
...
"""
        ...
        ex_desc_2 = f"""<success_rate>
The model succeeded in performing the task for {succ} times out of {total} trials. 
</success_rate>
...
Goal: Predict an difficulty in "easy" "medium" or"hard".
...
You will be provided with {k} records of previous rollouts. Use the records as examples to calibrate your internal scale. Do not restate these instructions in the answer.
"""
...
<rel>
- L2 distance of the test case with the nearest training data(blue cube and pink cube distance added): {test_record.get('l2_cm', '')}cm
- L2 distance of the blue cube in the test case with the nearest blue cube in the training data: {test_record.get('l2_big_cm', '')}cm
- L2 distance of the pink cube in the test case with the nearest pink cube in the training data: {test_record.get('l2_small_cm', '')}cm
</rel>

<pos>
- Position of the blue cube in the work space: ({test_record.get('big_pos', '')[0]}, {test_record.get('big_pos', '')[1]})
- Position of the pink cube in the work space: ({test_record.get('small_pos', '')[0]}, {test_record.get('small_pos', '')[1]})
</pos>

<failure>
Failure mode observed in similar test cases: {test_record.get("Reasons for failure", "")}
</failure>
...
"""
    ...
    retrive = retriever.retrieve(test_record, test_name, k, True)
    content.extend(build_examples_content(retrive[1], retrive[2]))
...
for i in range(queries_num):
    queries.append(build_message_for_query(all_records[i], all_test_names[i]))


def handle_query(i, query_msg, results, output_dir):
...
def main():
    ...
    output_dir = os.path.join(base_dir, timestamp + "_" + model_type + "_3")
    os.makedirs(output_dir, exist_ok=True)
    with open(os.path.join(output_dir, "queries.jsonl"), "w", encoding="utf-8") as fq:
        for q in queries:
            fq.write(json.dumps(q, ensure_ascii=False) + "\n")
    results = [handle_query(i, q, results, output_dir) for i, q in enumerate(queries)]
    ordered = [results.get(i, "NA") for i in range(len(queries))]
    with open(
        os.path.join(output_dir, "difficulties.json"), "w", encoding="utf-8"
    ) as f:
    ...
    all_case_re_path = os.path.join(base_dir, "full_case.json")
    ...
    all_results = {
        k: all_results[k] for k in sorted(all_results.keys(), key=lambda x: int(x))
    }
    with open(all_case_re_path, "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)
    with open(
        os.path.join(output_dir, "queries_result.json"), "w", encoding="utf-8"
    ) as f:
