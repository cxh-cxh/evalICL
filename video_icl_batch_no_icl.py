...
def build_examples_content(test_records, test_names):
    ...
    for i in range(len(test_records)):
        ...
        front_imgs = sample_video(
            os.path.join(ex["video_root"], ex["video"]["front"]),
            # sample_rate=30,
            num=10,
        )

        side_imgs = sample_video(
            os.path.join(ex["video_root"], ex["video"]["side"]),
            # sample_rate=30,
            num=10,
        )
        imgs = []
...
base_prompt_templetes = {
    "drawer": base_prompt_templete_drawer,
    "box": base_prompt_templete_box,
}
...
def build_message_for_query(args, test_record, test_name):
    ...
    side_imgs = sample_video(
        os.path.join(test_record["video_root"], test_record["video"]["side"]),
        # sample_rate=30,
        num=10,
    )
    imgs = []
